...
Ship = (function() {
  ...
  function Ship(camera, tunelDiameter) {
    this.camera = camera;
    this.tunelDiameter = tunelDiameter;
  ...
  Ship.prototype.update = function() {
    var currentDiameter, delta, ratio;
    delta = (Date.now() - this.lastUpdate) * 0.1;
    this.lastUpdate = Date.now();
    this.velocity.x += (-this.velocity.x) * 0.004 * delta;
    this.velocity.z += (-this.velocity.z) * 0.004 * delta;
    if (this.moveForward) {
    ...
    if (currentDiameter > this.tunelDiameter / 3) {
      ratio = (this.tunelDiameter / 3) / currentDiameter;
...
Space = (function() {
  ...
  function Space(threeJsScene, tunelDiameter) {
      ...
      for (orbitNumber = _i = 0, _ref = this.orbitsCount; _i < _ref; orbitNumber = _i += 1) {
        _results.push((function() {
          ...
          for (segmentNumber = _j = 0, _ref1 = this.segmentsCount; _j < _ref1; segmentNumber = _j += 1) {
            _results1.push(this.createParticleSystem(this.suborbitsDistance * (orbitNumber + (segmentNumber * this.segmentsCount))));
          }
          return _results1;
        }).call(this));
      }
...
Scene = (function() {
  ...
  function Scene() {
    ...
    tunelDiameter = 800;
    this.space = new Space(this.scene, tunelDiameter);
    this.ship = new Ship(this.camera, tunelDiameter);
